    ...
    <section className="mt-8">
      ...
      <div className="flex md:grid md:grid-cols-3 gap-4 overflow-x-auto pb-2 snap-x snap-mandatory md:overflow-visible scrollbar-none -mx-4 px-4 md:mx-0 md:px-0">
        {top3.map((g,i)=>{
          ...
          return (
            <Link key={g.id} to={`/game/${g.slug}`} className={`group relative overflow-hidden rounded border ${isFirst?"border-amber-400/30 bg-gradient-to-br from-amber-500/10 via-[#1B2838] to-[#1B2838]":"border-[#2A475E] bg-[#1B2838]"} p-4 hover:border-[#66C0F4]/50 transition snap-start shrink-0 w-[85%] md:w-auto`}>

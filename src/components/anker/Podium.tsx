  ...
  return (
    <section className="mt-8">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="font-display font-bold text-xl leading-none text-white">The Podium</h2>
          <p className="text-xs text-white/40">Top 3 all-time — bars scaled to #1 • Anker A-Z</p>
        </div>
      ...
      <div className="flex md:grid md:grid-cols-3 gap-4 overflow-x-auto pb-2 snap-x snap-mandatory md:overflow-visible scrollbar-none -mx-4 px-4 md:mx-0 md:px-0">
        {top3.map((g,i)=>{
          ...
          return (
            <Link key={g.id} to={`/game/${g.slug}`} className={`group relative overflow-hidden rounded-2xl border ${isFirst?"border-amber-400/30 bg-gradient-to-br from-amber-500/10 via-[#0F1424] to-[#0F1424]":"border-white/[0.06] bg-[#0F1424]"} p-4 hover:border-violet-500/30 transition snap-start shrink-0 w-[85%] md:w-auto`}>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="font-bold leading-tight line-clamp-2 group-hover:text-violet-300 text-white">{g.title}</div>
              ...
              <div className="mt-3 flex gap-2">
                ...
                <span className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-sm text-white">Details</span>
              </div>
            </Link>
          )
        })}

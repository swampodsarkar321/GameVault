...
export default function AnkerGameCard({ game, rank, total, showRank, variant="grid" }: Props){
  ...
  if(variant==="chart" && rank){
    // Chart row style like AnkerGames trending list
    ...
    return (
      <div className="group flex items-center gap-3 p-3 rounded-2xl bg-[#0F1424] border border-white/[0.06] hover:border-violet-500/30 hover:bg-[#141B31] transition">
        <div className="text-center w-10 shrink-0">
          <div className="font-display font-bold text-lg leading-none">{String(rank).padStart(2,"0")}</div>
          {stats && (
        ...
        <div className="flex-1 min-w-0">
          <Link to={`/game/${game.slug}`} className="font-semibold text-sm leading-tight line-clamp-1 hover:text-violet-300">{game.title}</Link>
        ...
        <div className="text-right shrink-0 hidden sm:block w-[110px]">
          <div className="text-sm font-bold leading-none">{stats ? formatDownloads(stats.weeklyDownloads) : game.size}</div>
  ...
  // Grid card - Anker style: cover top, then title, meta, size, two buttons
  return (
    <div className="group bg-[#0F1424] border border-white/[0.06] hover:border-violet-500/30 hover:bg-[#141B31] rounded-2xl overflow-hidden transition flex flex-col">
      <Link to={`/game/${game.slug}`} className="relative block overflow-hidden">
        {showRank && rank && (
          <div className="absolute top-2 left-2 z-10 w-7 h-7 rounded-full bg-black/70 backdrop-blur border border-white/15 grid place-items-center text-xs font-bold">
      ...
      <div className="p-3 flex-1 flex flex-col">
        <Link to={`/game/${game.slug}`} className="font-semibold leading-tight line-clamp-2 hover:text-violet-300 text-sm min-h-[40px]">{game.title}</Link>

...
export default function SoftwareCard({ software }: { software: Software }){
  ...
  return (
    <div className="group bg-[#0F1424] border border-white/[0.06] hover:border-violet-500/20 rounded-2xl p-3 flex flex-col transition">
      <Link to={`/software/${software.slug}`} className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="font-semibold text-sm line-clamp-1 text-white">{software.title}</div>
          <div className="text-xs text-white/50 line-clamp-1">{software.category} • {software.license}</div>
          <div className="flex items-center gap-2 mt-1 text-xs"><span className="px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-white/70">{software.version}</span><span className="text-white/40">{software.size}</span></div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 mt-3">
        <Link to={`/software/${software.slug}`} className="py-2 rounded-xl bg-white/[0.06] hover:bg-white/10 border border-white/10 text-center text-xs font-medium text-white">Details</Link>
        <Link to={`/download/${software.id}`} className="py-2 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-center text-xs font-semibold">Download</Link>
      </div>
    </div>
  )
}

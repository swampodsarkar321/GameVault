  ...
  return (
    <section className="relative overflow-hidden rounded-[28px] border border-white/[0.06] bg-gradient-to-br from-[#0F1424] via-[#101636] to-[#0F1424] p-6 md:p-10 mt-6">
      ...
      <div className="relative grid lg:grid-cols-[1.15fr_0.85fr] gap-8 items-center">
        <div>
          <div className="inline-flex items-center gap-2 text-xs font-semibold tracking-widest text-violet-300 bg-violet-500/10 border border-violet-500/20 rounded-full px-3 py-1"><Sparkles className="w-3.5 h-3.5"/> ANKER-STYLE VAULT • VERIFIED • FREEWARE ONLY</div>
          <h1 className="font-display font-bold text-[30px] md:text-[46px] leading-[0.95] mt-4">GameVault<br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-300">Discover. Explore. Download.</span></h1>
          <p className="text-white/60 mt-3 max-w-xl leading-relaxed text-sm md:text-[15px]">AnkerGames-inspired hub — curated freeware & open-source titles, fast verified sources. No pirated content.</p>
          {/* Anker-style search bar */}
          <form onSubmit={onSearch} className="mt-6 flex items-center gap-2 bg-white/[0.07] border border-white/10 rounded-2xl p-2 max-w-xl focus-within:border-violet-500/50 focus-within:bg-white/[0.09] transition">
            ...
            <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search GTA V, Valorant, indie gems..." className="flex-1 bg-transparent outline-none text-sm placeholder:text-white/35 py-2" />

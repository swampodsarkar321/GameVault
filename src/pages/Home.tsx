...
export default function Home(){
  ...
  useEffect(()=>{ setPageMeta("Free PC Games Download | Freeware Vault","GameVault — free PC games download. Curated freeware, open-source & demos. Anker-style vault with Trending & Top charts. 100% legal, verified sources. Download free PC games.", { canonical:"/", keywords:"free PC games download, freeware games, open source games, free games download, PC games free, trending free games, top free PC games" }); Promise.all([fetchGames({limitN:20}), fetchSoftware({limitN:8})]).then(([g,s])=>{ setGames(g); setSoftware(s); setLoading(false)}) },[])
  ...
  return (
    <div className="pb-6">
      ...
      {/* Featured - Anker grid style */}
      <section className="mt-8">
        <div className="flex items-end justify-between"><h2 className="font-display font-bold text-xl">Featured Games <span className="text-xs font-normal text-white/30 ml-2">Anker-style vault</span></h2><Link to="/games" className="text-sm text-violet-300 hover:text-white">View all →</Link></div>
      ...
      <section className="mt-8">
        <div className="flex items-end justify-between"><h2 className="font-display font-bold text-xl">Popular Games</h2><Link to="/top-games" className="text-sm text-violet-300 hover:text-white">View top →</Link></div>

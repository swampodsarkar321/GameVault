...
export default function Home(){
  ...
  const cats = [
    {name:"Action", slug:"action"},{name:"Adventure", slug:"adventure"},{name:"RPG", slug:"rpg"},{name:"Strategy", slug:"strategy"},
    {name:"Development", slug:"development"},{name:"Graphics", slug:"graphics"},{name:"Audio", slug:"audio"},{name:"Security", slug:"security"},
  ]
  return (
    <div className="pb-6">
      <Hero/>
      ...
      <section className="mt-8">
        <h2 className="font-display font-bold text-xl">Browse Categories</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-4">{cats.map(c=> <CategoryCard key={c.slug} name={c.name} slug={c.slug}/>)}</div>
      </section>
